...
class TestJsonRepository:
    ...
    @pytest.fixture
    def user_repo(
        self, tmp_path: Path, sample_user: User
    ) -> JsonUserRepository:
    ...
    def test_get_by_id_returns_user(
        self, user_repo: JsonUserRepository, sample_user: User
    ):
    ...
    def test_add_persists_user(self, tmp_path: Path, sample_user: User):
        file_path = tmp_path / "users.json"
        user_repo = JsonUserRepository(file_path)
    ...
    def test_delete_removes_user(
        self, path: Path, user_repo: JsonUserRepository, sample_user: User
    ):
